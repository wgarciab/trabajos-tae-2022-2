...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
dictionary %>% filter(`VARIABLE NAME` %in% names(college_subset)) %>% select('NAME OF DATA ELEMENT', 'VARIABLE NAME')
```
...
De acuerdo al metodo de Elbow, el número ideal de clusters es 9, ya que se puede apreciar un "codo" en la gráfica cerca a k = 9.
...
```
### Propuesta de analisis en colombia 

Para el desarrollo de un análisis en colombia, similar al realizado con el dataset College
ScoreCard podemos extraer información de las siguientes páginas:
